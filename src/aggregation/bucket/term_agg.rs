...
pub(crate) fn build_segment_term_collector(
    ...
    let (use_vec_buckets, num_terms_for_vec) = {
        let terms_req_data = req_data.get_term_req_data(accessor_idx);
        ...
        if can_use_vec {
            if let Some(str_col) = terms_req_data.str_dict_column.as_ref() {
                let num_terms = str_col.num_terms();
                let within_threshold = num_terms <= 1_024;
                (
                    within_threshold,
                    if within_threshold { num_terms } else { 0 },
                )
            } else {
                (false, 0)
            }
